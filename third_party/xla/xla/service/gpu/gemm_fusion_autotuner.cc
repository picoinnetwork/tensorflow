...
bool IsOnlyDynamicSliceIndex(const HloInstruction& operand) {
  // It is not a dynamic slice index if it has no users.
  if (operand.user_count() == 0) {
    return false;
  }
  // Operands with multiple users can be removed if all users are dynamic-slice
  // and they are not the array being sliced.
...
absl::StatusOr<std::unique_ptr<HloModule>> TritonGemmAutotuneExtractor(
  ...
  TF_RETURN_IF_ERROR(
      ReplaceDynamicSliceWithSliceInTheComputationOfTheRootFusion(*new_module));
...
absl::Status ReplaceDynamicSliceWithSliceInTheComputationOfTheRootFusion(
    HloModule& module) {
  ...
  // The fusion computation will have dynamic slice instructions removed.
  ...
  for (int i = 0; i < root->operand_count(); ++i) {
    const HloInstruction& computation_param =
